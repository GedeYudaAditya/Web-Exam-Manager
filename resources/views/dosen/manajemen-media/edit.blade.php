...
@section('content')
    <div class="container mt-20">
        ...
        <div class="container rounded shadow-md p-10 mb-10 bg-white">
            <form action="{{ route('dosen.media.update', $video->slug) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="container flex flex-col">
                    ...
                    <textarea name="description" id="deskripsi" cols="30" rows="10" class="border-2 rounded-md p-2 mb-5">{{ old('description') ?? $video->description }}</textarea>
                    ...
                    <div class="flex flex-col mb-5">
                        ...
                        <input type="text" name="embed" id="video" class="border rounded p-2"
                            value="{{ old('embed') ?? $video->embed }}" onchange="previewVideo()">
                    </div>
                    ...
                    <select name="category" id="category" class="border-2 rounded-md p-2 mb-5">
                        ...
                        <option value="paru" {{ $video->category == 'paru' ? 'selected' : '' }}>Paru</option>
                        <option value="ginjal" {{ $video->category == 'ginjal' ? 'selected' : '' }}>Ginjal</option>
                        <option value="reproduksi" {{ $video->category == 'reproduksi' ? 'selected' : '' }}>Reproduksi
                        </option>
                    </select>
                    ...
                    <button type="submit" class="button">Edit Video</button>
                </div>
            </form>
        </div>
    </div>
